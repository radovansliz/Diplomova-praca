...
def count_records_per_family_in_directory(input_dir):
    # Získanie zoznamu všetkých CSV súborov v danom adresári, ktoré obsahujú "before" v názve
    all_csv_files = glob.glob(os.path.join(input_dir, "*merged*.csv"))

    # Overenie, či sa našli nejaké CSV súbory
    if not all_csv_files:
        print(f"Nenašli sa žiadne CSV súbory v adresári: {input_dir} obsahujúce 'before' v názve.")
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prejde všetky CSV súbory obsahujúce 'before' v názve v zadanom adresári a vypíše unikátne hodnoty zo stĺpca 'Family' s počtom záznamov 1000 a viac.")
