...
lgbm_model = LGBMClassifier(
    boosting_type='gbdt',    # Typ boostingu (gradient boosting)
    n_estimators=100,        # Počet stromov
    max_depth=-1,            # Maximálna hĺbka stromov
    learning_rate=0.1,       # Rýchlosť učenia
...
# Krížová validácia
cv_scores = cross_val_score(lgbm_model, X_train, y_train, cv=5)  # 5-násobná krížová validácia
...
def evaluate_model(model, X_val, y_val, X_test, y_test):
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("accuracy_curve_lgbm.png")
    plt.close()


# Zavolanie funkcie na vyhodnotenie
evaluate_model(lgbm_model, X_val, y_val, X_test, y_test)
