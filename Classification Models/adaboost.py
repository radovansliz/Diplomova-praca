...
adaboost_model = AdaBoostClassifier(
    base_estimator=DecisionTreeClassifier(max_depth=1, random_state=42),  # Základný model (Decision Stump)
    n_estimators=50,            # Počet iterácií boostingu (hyperparameter na ladenie)
    learning_rate=1.0,          # Rýchlosť učenia (zníženie príspevku každého slabého klasifikátora)
    random_state=42
)
...
# Krížová validácia
cv_scores = cross_val_score(adaboost_model, X_train, y_train, cv=5)  # 5-násobná krížová validácia
...
evaluate_model(adaboost_model, X_val, y_val, X_test, y_test)
