...
base_estimator = DecisionTreeClassifier(
    max_depth=None,  # Maximálna hĺbka stromu (hyperparameter na ladenie)
    random_state=42
)

# Definícia Bagging Classifier
bc_model = BaggingClassifier(
    base_estimator=base_estimator,  # Použitie rozhodovacieho stromu ako základného modelu
    n_estimators=100,              # Počet slabých klasifikátorov (hyperparameter na ladenie)
    max_samples=1.0,               # Podiel vzoriek pre každý model
    max_features=1.0,              # Podiel vlastností pre každý model
    bootstrap=True,                # Bootstrapovanie vzoriek
    bootstrap_features=False,      # Bez bootstrapovania vlastností
    random_state=42
)
...
# Krížová validácia
cv_scores = cross_val_score(bc_model, X_train, y_train, cv=5)  # 5-násobná krížová validácia
...
def evaluate_model(model, X_val, y_val, X_test, y_test):
    ...
    # 3. Loss krivka - BaggingClassifier nemá loss priamo, preto simulujeme
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("accuracy_curve_bc.png")
    plt.close()
