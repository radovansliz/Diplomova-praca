...
def run_rf_pipeline(X, y, config, evaluate_flag=True):
    ...
    cv_scores = cross_val_score(rf_model, X_train, y_train, cv=config["cross_validation_n"])
    ...
    if evaluate_flag:
        evaluate_model(rf_model, X_train, y_train, X_test, y_test, config, val_accuracy, test_accuracy, cv_scores, FP_val, FN_val, TP_val, TN_val, FPR_val, FP_test, FN_test, TP_test, TN_test, FPR_test)
...
if __name__ == "__main__":
    ...
    run_rf_pipeline(X, y, config, evaluate_flag=EVALUATE_MODEL)
